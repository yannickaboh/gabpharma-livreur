...
- **Géolocalisation temps réel du livreur implémentée et vérifiée le 4 septembre 2026** (backend + Flutter Livreur, dans les deux dépôts) — ping de position toutes les 45 s en premier plan pendant une course active (`assigned`/`picked_up`/`in_transit`), exposée côté Patient avec péremption à 5 minutes. Voir `branchement_livreur.md` §5. Reste à faire (hors périmètre de cette session) : afficher le point sur la carte côté app Patient (dépôt `gabpharma_patient`).
- **Prochain module à brancher** : Carte et navigation (écran 09, bouton « ouvrir navigation externe » uniquement — la carte `CustomPainter` peut maintenant afficher la vraie position du livreur si le SDK cartographique est branché), puis Historique/Revenus/Zones selon l'ordre de `api_contrat_besoins.md` §7.
