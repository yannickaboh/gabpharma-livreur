...
| Documents | ✅ **Branché et vérifié le 10 septembre 2026** — dossier organisé par emplacement (`required_slots`, dépend du type de pièce d'identité et du véhicule), pas par type de document comme le mockup le supposait. Sélecteur de type de pièce ajouté, upload photo réel (`image_picker` + multipart), document « Assurance » retiré |
| Notifications | ✅ **Branché et vérifié le 10 septembre 2026** — « Document validé »/« Versement reçu » retirés (aucune source backend), seules les 3 sources réelles restent (course dispo, changement de statut, incident résolu). État lu/non lu entièrement local (`unread_count` toujours `null` côté API, flux jamais persisté) |
| Centre d'aide / tickets / conversation | ✅ **Branché et vérifié le 10 septembre 2026** — les 3 catégories rapides correspondent 1:1 aux codes réels, pièce jointe réellement câblée (upload multipart, mieux que côté Patient), tickets résolus restent consultables/répondables sans réouverture automatique |
| Profil et sécurité | ✅ **Branché et vérifié le 10 septembre 2026** — identité/téléphone réels (`GET /mobile/profile/`), changement de mot de passe réel (`POST /mobile/profile/password/`, testé échec et succès). Véhicule en lecture seule confirmé cohérent avec notre écran (modification réservée au Staff) ; plaque d'immatriculation inventée du mockup retirée (aucun champ backend). **Édition des informations personnelles ajoutée le même jour** (`PATCH /mobile/profile/`, sur demande explicite hors mockup) — voir `branchement_livreur.md` §14.1. Photo de profil laissée hors périmètre : aucun champ avatar côté backend (ni Livreur, ni Patient) |
...
| 14 Documents | `GET /mobile/courier/verification/`, `POST .../identity-type/`, `POST .../documents/`, `POST .../submit/` | ✅ **Branché et vérifié le 10 septembre 2026.** Assurance retirée (aucune catégorie backend pour elle). Écran restructuré autour des `required_slots` réels (dépendent du type de pièce d'identité choisi et du véhicule) plutôt que de 3 cartes fixes par type de document — un sélecteur de type de pièce (CNI/carte de séjour/passeport, absent du mockup) a dû être ajouté en préalable. Première prise de photo réelle + upload `multipart/form-data` du projet (`image_picker`, `ApiClient.postMultipart()`). Dossier verrouillé une fois soumis (`pending_review`/`approved`). Voir `branchement_livreur.md` §11. |

### Lot 4 — Communication et compte

| Écran | Endpoints nécessaires | État |
|---|---|---|
| 15 Notifications | `GET /mobile/notifications/` | ✅ **Branché et vérifié le 10 septembre 2026.** Le flux agrégé livreur (`_courier_notifications`, `apps/api/mobile_shared.py`) ne produit que 3 types d'événements réels : course `awaiting_assignment` compatible disponible (si en ligne), changement de statut d'une course affectée (`DeliveryStatusHistory`), incident résolu. « Document validé » et « Versement reçu » (écran 15 démo) retirés — aucune source backend pour eux. **Découverte confirmée sur le payload réel avant codage** : `unread_count` toujours `null`, `persistence: "aggregated"` — le flux est recalculé à la volée, jamais persisté, donc aucune notification n'a d'id stable et l'état lu/non lu reste entièrement local à l'app (réinitialisé à chaque rechargement). « Voir les détails » limité à la seule notification « course disponible » (`/available-detail`) : aucune route de détail générique par id n'existe pour les autres événements `delivery`. Voir `branchement_livreur.md` §12. |
| 16 Centre Support | `GET/POST /mobile/support/tickets/` | ✅ **Branché et vérifié le 10 septembre 2026.** Les 3 boutons rapides (Livraison/Compte/Paiement) correspondent 1:1 aux codes réels (`delivery`/`account`/`payment`) — `GET /mobile/support/categories/` non appelé, aucune valeur dynamique à en tirer. Champ "Objet" ajouté (absent du mockup, requis côté backend) ; "commande liée" volontairement omis (Option A, contrairement à l'app Patient). Pièce jointe réellement câblée à la création (upload multipart réel, `image_picker`). Indicateur "EN RETARD" redéfini comme un booléen `is_sla_overdue` indépendant du statut réel, plus une 6ᵉ valeur de statut fictive. Voir `branchement_livreur.md` §13. |
| 17 Conversation Support | `GET /mobile/support/tickets/<id>/`, `POST .../reply/` | ✅ **Branché et vérifié le 10 septembre 2026.** Notes internes Staff jamais exposées (déjà filtrées côté serveur). Pièce jointe réellement câblée à la réponse (écart positif vs l'app Patient, restée honnêtement "indisponible" faute de client multipart lors de son propre branchement). Persona statique "Sarah du support technique" retirée au profit du vrai `author_name` par message. Accusés de lecture retirés (aucun champ `is_read` côté API). Tickets résolus/fermés restent consultables et répondables (bandeau honnête "ne se rouvre pas automatiquement"), confirmé en conditions réelles : une réponse sur un ticket résolu laisse son statut inchangé. Voir `branchement_livreur.md` §13. |
| 18 Profil et sécurité | `GET /mobile/profile/`, `POST /mobile/profile/password/` | ✅ **Branché et vérifié le 10 septembre 2026.** Identité (nom, téléphone) et mot de passe réels — le formulaire `ChangePasswordScreen` avait déjà exactement les 3 champs requis côté backend (y compris le mot de passe actuel). Le champ « Véhicule » (lecture seule dans notre app, dialogue « à modifier auprès du Staff ») confirmé exactement cohérent avec la réalité : `vehicle_type_label` réutilisé depuis `/availability/` (aucun champ véhicule sur `/mobile/profile/`), **aucune route ne permet de le modifier depuis le mobile** — bon alignement, rien à changer. Plaque d'immatriculation inventée du mockup ("GA-204-LB") retirée : aucun champ backend pour un numéro d'immatriculation. `PATCH /mobile/profile/` et `POST /mobile/profile/deactivate/` (désactivation réversible du compte, réactivation à la reconnexion) restent disponibles côté API mais non utilisés : aucune UI d'édition ni de désactivation dans le mockup 18 — voir §6. Voir `branchement_livreur.md` §14. |
...
8. ~~**Documents**~~ — **fait le 10 septembre 2026** : document « Assurance » retiré (§6.3), vrai découpage recto/verso par emplacement adopté, sélecteur de type de pièce ajouté, upload photo réel branché — voir `branchement_livreur.md` §11.
9. ~~**Notifications**~~ — **fait le 10 septembre 2026** : flux agrégé branché tel quel (course dispo/statut/incident résolu), « Document validé »/« Versement reçu » retirés (§6.7) ; état lu/non lu local à l'app (aucune persistance backend, `unread_count` toujours `null`) — voir `branchement_livreur.md` §12.
10. ~~**Centre d'aide, tickets, conversation support**~~ — **fait le 10 septembre 2026** : les 3 catégories rapides correspondaient déjà 1:1 aux codes réels, aucune réconciliation nécessaire au-delà du mapping trivial ; pièce jointe réellement câblée (upload multipart) — voir `branchement_livreur.md` §13.
11. ~~**Profil et sécurité**~~ — **fait le 10 septembre 2026** : identité/téléphone/mot de passe réels, véhicule en lecture seule (déjà cohérent avec l'API), plaque d'immatriculation inventée retirée — voir `branchement_livreur.md` §14. `POST /mobile/profile/deactivate/` reste disponible côté API mais non exposé (non demandé par le mockup 18).
